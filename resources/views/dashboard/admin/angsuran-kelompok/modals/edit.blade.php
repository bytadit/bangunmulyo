<div class="modal fade" id="editDataAngsuranKelompok{{ $angsuran->id }}" tabindex="-1" aria-labelledby="modalEditDataAngsuranKelompok">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalEditDataAngsuranKelompok">Ubah Riwayat Angsuran Kelompok
                    {{ $kelompok_name }} Periode ke - {{ $periode }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form method="post" enctype="multipart/form-data"
                    action="{{ route('riwayat-angsuran-kelompok.update', ['kelompok' => $kelompok, 'pinjaman_kelompok' => $pinjaman_kelompok, 'riwayat_angsuran_kelompok' => $angsuran->id]) }}">
                    @csrf
                    @method('put')
                    <div class="row g-3">
